...
void DijkstraPlanner::configure(const rclcpp_lifecycle::LifecycleNode::WeakPtr & parent, std::string name, 
                ...
                std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
    ...
    if (!smooth_client_->wait_for_action_server(std::chrono::seconds(3))){
        RCLCPP_ERROR(node_->get_logger(),"Action server not available waiting");
    }
}
...
PLUGINLIB_EXPORT_CLASS(alphabot_planning::DijkstraPlanner, nav2_core::GlobalPlanner)