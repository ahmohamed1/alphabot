...
#include "alphabot_motion/pd_motion_planner.hpp"
#include "tf2/utils.h"
#include "geometry_msgs/msg/transform_stamped.hpp"
#include "tf2_geometry_msgs/tf2_geometry_msgs.hpp"

namespace alphabot_motion
{
    ...
    prev_linear_error_(0.0), prev_angular_error_(0.0)
{
    ...
    path_sub_ = create_subscription<nav_msgs::msg::Path>(
        "/astar/path", 10, std::bind(&PDMotionPlanner::pathCallback, this, std::placeholders::_1));
...
void PDMotionPlanner::controlLoop()
{
    ...
    // RCLCPP_INFO(get_logger(), "frame_id Global Pose: %s", global_plane_.header.frame_id.c_str());
    ...
    cmd_vel.linear.x  = std::clamp(kp_ * linear_error + kd_ * linear_error_derivative, -max_linear_velocity_, max_angular_velocity_);
...
{
    if(global_plane_.header.frame_id == frame){
        return true;
    }
    ...
    try
    {
        transform = tf_buffer_->lookupTransform(frame, global_plane_.header.frame_id, tf2::TimePointZero);
    }
    catch(tf2::TransformException & ex)
    {
        RCLCPP_ERROR_STREAM(get_logger(), 
        "Could not transform plan from frame"<< global_plane_.header.frame_id<<" to " << frame);
        return false;
    }
    

    for(auto & pose: global_plane_.poses){
        tf2::doTransform(pose, pose, transform);
    }
    global_plane_.header.frame_id = frame;
    return true;
}

geometry_msgs::msg::PoseStamped PDMotionPlanner::getNextPose(const geometry_msgs::msg::PoseStamped & robot_pose)
{
    auto next_pose = global_plane_.poses.back();
    for(auto pose_it = global_plane_.poses.rbegin(); pose_it != global_plane_.poses.rend(); ++pose_it){
...
int main(int argc, char **argv)
{
    ...
    auto node = std::make_shared<alphabot_motion::PDMotionPlanner>();
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}