...
void PDMotionPlanner::configure(const rclcpp_lifecycle::LifecycleNode::WeakPtr & parent,
...
{
    RCLCPP_INFO(logger_ ,"Activating up plugin PDMotionPlanner");
    last_cycle_time_ = clock_->now();
}
void PDMotionPlanner::deactivate()
{
    RCLCPP_INFO(logger_ ,"Deactivating up plugin PDMotionPlanner");
}
  ...
  nav2_core::GoalChecker *)
{
    ...
    cmd_vel.twist.linear.x  = std::clamp(kp_ * linear_error + kd_ * linear_error_derivative, -max_linear_velocity_, max_angular_velocity_);
...
bool PDMotionPlanner::transformPlan(const std::string & frame)
{
    ...
    catch(tf2::TransformException & ex)
    {
        ...
        "Could not transform plan from frame"<< global_plan_.header.frame_id<<" to " << frame);
        return false;
    }
...
PLUGINLIB_EXPORT_CLASS(alphabot_motion::PDMotionPlanner, nav2_core::Controller)