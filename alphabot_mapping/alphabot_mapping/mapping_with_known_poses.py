...
def poseOnMap(pose: Pose, map_info: MapMetaData):
    return (
        pose.x < map_info.width
        and pose.x >= 0
        and pose.y < map_info.height
        and pose.y >= 0
    )
...
class MappingWithKnownPoses(Node):
    def __init__(self, name):
        ...
        self.probability_map_ = [prob2logodds(PRIOR_PROB)] * (
            self.map_.info.width * self.map_.info.height
        )

        self.tf_buffer = Buffer()
        self.tf_listener = TransformListener(self.tf_buffer, self)
        self.scan_sub = self.create_subscription(
            LaserScan, "scan", self.scanCallback, 10
        )
    ...
    def scanCallback(self, scan: LaserScan):
        try:
            ...
            self.get_logger().error(
                "Unable to transform between /odom and /base_footprint"
            )
            return

        robot_p = coordinatesToPose(
        ...
        for i in range(len(scan.ranges)):
            ...
            for pose, value in poses:
                if poseOnMap(pose, self.map_.info):
                    ...
                    self.probability_map_[cell] += prob2logodds(value) - prob2logodds(
                        PRIOR_PROB
                    )

    def timerCallback(self):
        self.map_.data = [
            int(logodds2prob(value) * 100) for value in self.probability_map_
        ]
